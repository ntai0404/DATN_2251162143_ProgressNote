...
class PDFProcessor:
    ...
    def extract_text_by_article(self, pdf_path, use_cloud=True):
        ...
        if len(full_text.strip()) < 100:
            ...
            # Tính toán tên file an toàn (ASCII) do Bot đã đổi tên
            ...
            if chandra_file.exists():
                ...
                try:
                    ...
                    subprocess.run([sys.executable, str(bot_script), pdf_path], check=True)
        ...
        if len(parts) > 1:
            ...
            for i in range(1, len(parts), 2):
                ...
                enriched_content = f"[Văn bản: {filename}] [Bối cảnh: {current_section}] [{article_header}]: {cleaned_content}"
                
                chunks.append({
                    "title": f"{filename} - {article_header}",
                    "content": enriched_content,
                    "raw_content": cleaned_content, # Giữ raw để hiển thị cho User
                    "metadata": {
